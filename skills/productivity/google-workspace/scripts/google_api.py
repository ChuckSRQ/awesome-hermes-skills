...
Usage:
  ...
  python google_api.py contacts create --name "Name" --phone "+1234567890" [--phone-label personal]
  python google_api.py contacts create --name "Name" --email "a@b.com" [--email-label work]
...
def contacts_create(args):
    ...
    if args.phone:
        body["phoneNumbers"] = [{"value": args.phone, "type": args.phone_label or "mobile"}]
    if args.email:
        body["emailAddresses"] = [{"value": args.email, "type": args.email_label or "home"}]
    result = service.people().createContact(body=body).execute()
    print(json.dumps({"status": "created", "resourceName": result.get("resourceName")}, indent=2))
...
def main():
    ...
    pc = con_sub.add_parser("create")
    pc.add_argument("--name", required=True, help="Full name or given name")
    pc.add_argument("--phone", help="Phone number (e.g. +1234567890)")
    pc.add_argument("--phone-label", default="mobile", help="Phone label (default: mobile)")
    pc.add_argument("--email", help="Email address")
    pc.add_argument("--email-label", default="home", help="Email label (default: home)")
    pc.set_defaults(func=contacts_create)
