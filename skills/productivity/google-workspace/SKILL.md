...
>    Google Drive API, Google Sheets API, Google Docs API, Google Slides API,
>    Google Forms API, People API
...
**Important on scopes:** The default SCOPES list in `setup.py` now includes all major Workspace APIs
(Gmail, Calendar, Drive, Sheets, Docs, Slides, Forms, Contacts). This means a single OAuth
consent flow covers everything — Slides presentations, full Drive read/write, etc. If you ever
need to add a scope, edit `SCOPES` in `setup.py` before running `--auth-url`, then redo the
auth flow. The token stored at `google_token.json` will be incomplete until re-authenticated
...
**Important on scopes:** The People API `connections` endpoint only reads from "My Contacts" (not "Other Contacts"). If `contacts list` returns empty despite the account having contacts, either the list is genuinely empty or contacts live in "Other Contacts" which requires the full `contacts` scope (not `contacts.readonly`). The default `setup.py` installs `contacts.readonly`. To enable write operations (create/update/delete contacts), change `contacts.readonly` to `contacts` in both `setup.py` and `google_api.py`, then re-authenticate (`--revoke` then repeat auth steps).
